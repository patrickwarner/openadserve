...
func run(logger *zap.Logger, cfg config.Config) error {
	...
	logger.Info("using in-memory ad data store")
	...
	analyticsSvc, err := analytics.InitClickHouse(
		cfg.ClickHouseDSN,
		pg,
		metricsRegistry,
		cfg.CHMaxOpenConns,
		cfg.CHMaxIdleConns,
		cfg.CHConnMaxLifetime,
		cfg.CHConnMaxIdleTime,
	)
	if err != nil {
	...
	// Connect analytics to the data store for spend updates
	analyticsSvc.SetAdDataStore(adDataStore)
	...
	// Serve minified SDK in production, original in development
	r.PathPrefix("/static/").Handler(http.StripPrefix("/static/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Intercept SDK requests to serve minified version in production
		if r.URL.Path == "/sdk/adsdk.js" && os.Getenv("GO_ENV") == "production" {
			http.ServeFile(w, r, "./static/sdk/adsdk.min.js")
			return
		}
	...
	// Add trace logger middleware (adds trace IDs to all logs automatically)
	handler = middleware.WithTraceLogger(logger)(handler)

	// Wrap with OpenTelemetry instrumentation if tracing is enabled
	if cfg.TracingEnabled {
	...
	if cfg.ReloadInterval > 0 {
		logger.Info("auto-reload enabled for in-memory store", zap.Duration("interval", cfg.ReloadInterval))
	...
	// Log sampling statistics every 5 minutes
	samplingTicker := time.NewTicker(5 * time.Minute)
	go func() {
		for {
			select {
			case <-samplingTicker.C:
				observability.LogSamplingStats(logger)
				observability.ResetSamplingStats() // Reset to get fresh stats for next period
			case <-ctx.Done():
				samplingTicker.Stop()
				return
			}

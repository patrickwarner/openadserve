...
type AnalyticsService interface {
	...
	RecordEvent(ctx context.Context, store models.AdDataStore, eventType, requestID, impID, creativeID string, lineItemID int, cost float64, targetingCtx models.TargetingContext, publisherID int) error
	// RecordImpression is a convenience wrapper for impression events.
	RecordImpression(ctx context.Context, store models.AdDataStore, requestID, impID, creativeID string, lineItemID int, deviceType, country string, publisherID int) error
	// RecordClick is a convenience wrapper for click events and CPC spend.
	RecordClick(ctx context.Context, store models.AdDataStore, requestID, impID, creativeID string, lineItemID int, deviceType, country string, publisherID int) error
}
...
func (a *Analytics) RecordEvent(ctx context.Context, store models.AdDataStore, eventType, requestID, impID, creativeID string, lineItemID int, cost float64, targetingCtx models.TargetingContext, publisherID int) error {
	if a == nil || a.DB == nil {
	...
	stmt := `INSERT INTO events (timestamp, event_type, request_id, imp_id, creative_id, campaign_id, line_item_id, cost, device_type, country, publisher_id, key_values) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	if _, err := a.DB.ExecContext(ctx, stmt, time.Now(), eventType, requestID, impID, cr, cmp, li, cost, dt, co, pub, keyValues); err != nil {
...
func (a *Analytics) RecordImpression(ctx context.Context, store models.AdDataStore, requestID, impID, creativeID string, lineItemID int, deviceType, country string, publisherID int) error {
	...
	if err := a.RecordEvent(ctx, store, "impression", requestID, impID, creativeID, lineItemID, cost, targetingCtx, publisherID); err != nil {
...
// RecordClick is a convenience wrapper for click events and CPC spend.
func (a *Analytics) RecordClick(ctx context.Context, store models.AdDataStore, requestID, impID, creativeID string, lineItemID int, deviceType, country string, publisherID int) error {
	var li *models.LineItem
	if lineItemID > 0 {
		li = models.GetLineItemByID(store, lineItemID)
	}
	...
	if err := a.RecordEvent(ctx, store, "click", requestID, impID, creativeID, lineItemID, cost, targetingCtx, publisherID); err != nil {
		if li != nil && li.BudgetType == models.BudgetTypeCPC {
			li.Spend += li.CPC
			a.Metrics.SetSpendTotal(strconv.Itoa(li.CampaignID), li.Spend)
			a.saveSpend(li)
...
func (a *Analytics) GetEventsByRequestID(id string) ([]EventRecord, error) {
	...
	query := `SELECT timestamp, event_type, request_id, imp_id, creative_id, campaign_id, line_item_id, cost, device_type, country, publisher_id FROM events WHERE request_id=? ORDER BY timestamp`
	...
	for rows.Next() {
		var ev EventRecord
		if err := rows.Scan(&ev.Timestamp, &ev.EventType, &ev.RequestID, &ev.ImpID, &ev.CreativeID, &ev.CampaignID, &ev.LineItemID, &ev.Cost, &ev.DeviceType, &ev.Country, &ev.PublisherID); err != nil {
